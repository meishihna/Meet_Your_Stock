...
// 關注清單頁:列出所有右滑(關注)與上滑(深入研究)的公司。

import Link from "next/link";
import { useMemo, useState } from "react";
import { AnimatePresence } from "framer-motion";
import { getCompanyByTicker } from "@/lib/dataService";
  ...
  const items = useMemo(
    () =>
      watchlist
        .map((s) => ({
          record: s,
          company: getCompanyByTicker(s.ticker),
        }))
        .filter((x) => x.company),
    [watchlist]
  );
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400">
                    市值 {formatMarketCap(c.fundamentals.marketCap)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-semibold">{formatMoney(c.price.current)}</p>
                  <p
                    className={`text-sm ${
